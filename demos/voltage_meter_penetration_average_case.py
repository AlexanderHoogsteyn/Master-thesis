...
methods = ["voltage Pearson correlation","power Pearson correlation ","power/voltage conform method","voltage assisted power based method","voltage assisted power based method smart"]
...
for case, feeder_id in enumerate(included_feeders):
    for rep in range(0, reps):
        ...
        for i, voltage_pen in enumerate(voltage_pen_range):
            feeder = Feeder(feederID=feeder_id, include_three_phase=include_three_phase,error_class=ErrorClass(accuracy, s=s))
            result = voltage_assisted_power_based_method(feeder, nb_salient_components=nb_salient_components,length=length,voltage_meter_penetration=voltage_pen)
            col += [result]
        scores.append(col)
        ...
        for i, voltage_pen in enumerate(voltage_pen_range):
            feeder = Feeder(feederID=feeder_id, include_three_phase=include_three_phase,error_class=ErrorClass(accuracy, s=s))
            result = voltage_assisted_power_based_method(feeder, nb_salient_components=nb_salient_components,length=length,voltage_meter_penetration=voltage_pen,select_biggest=True)
            col += [result]
        scores.append(col)
